...
void usage( const char* name )
{
  ...
  cerr << "dir: directory o� seront �crites les images .eps\n\n";
		    ...
		    unsigned lpf, unsigned cpf )
{
  ...
  for( i=0; i<nfl; ++i )
	...
	lines.push_back( "0 0" );	// lignes du milieu
      lines.push_back( "0 1" );		// derni�re ligne du sillon
    }
		 ...
		 const string & psfile )
{
  ...
  //	fichiers de donn�es pour gnuplot
  ...
  c = 0;
  ind = 0;
  for( j=0; j<3; ++j )	// s�ries de graphes
    {
      serr = 0;
      for( i=0; i<gr[j]; ++i )
      ...
      for( i=0; i<tables[tbl]; ++i )			// lignes
	{
	  ...
	  for( k=0; k<linespercell; ++k )
	      ...
	      for( j=0; j<(int)text.size(); ++j )
		{
	      ...
	      for( ; j<ncols; ++j )	// compl�ter les lignes vides
		{
		  for( c=0; c<colspercell; ++c )
		    {
		      f << "<cell>\n\\begin_inset Text\n";
