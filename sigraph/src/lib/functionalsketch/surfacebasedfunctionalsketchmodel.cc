...
SurfaceBasedFunctionalSketchSimilarityModel::SurfaceBasedFunctionalSketchSimilarityModel( float simweight, float simdistance )
  : Model(), _simweight( simweight ), _simdistance(simdistance)
{
//   cout << "TEST2 : " << simdistance << endl;
}


SurfaceBasedFunctionalSketchSimilarityModel::~SurfaceBasedFunctionalSketchSimilarityModel()
{
}


SurfaceBasedFunctionalSketchSimilarityModel &
    SurfaceBasedFunctionalSketchSimilarityModel::operator =
    ( const SurfaceBasedFunctionalSketchSimilarityModel & x )
{
  *(Model *) this = x;
  _simweight = x._simweight;
  _simdistance = x._simdistance;
  cout << "ARG :" << _simdistance << endl;
  return *this;
}


Model* SurfaceBasedFunctionalSketchSimilarityModel::clone() const
{
  return new SurfaceBasedFunctionalSketchSimilarityModel( *this );
}


double SurfaceBasedFunctionalSketchSimilarityModel::prop( const Clique* cl )
{
  // cout << "SurfaceBasedFunctionalSketchSimilarityModel::prop\n";
...
//   // Test : have we computed overlaps yet ? If not let's do it
  carto::rc_ptr<SurfaceBasedOverlapMap> overlap;
  //SurfaceBasedOverlapMap *overlap;
  int index1, index2;
...
//   float                    distance, tmin1, tmin2, tmax1, tmax2;
  ...
  overlap.reset(new SurfaceBasedOverlapMap);
      //overlap=new SurfaceBasedOverlapMap;
  ...
  //DEBUG
  //cout << "SurfaceBasedFunctionalSketchSimilarityModel::prop" << endl;
...
//           if (rec > _simdistance) pot = 0.0;
//           else  
//           pot = _simweight/_simdistance * rec - _simweight;
          float sigma = 10.0/sqrt(2*log(1/0.1));          // paramètre de la gaussienne : 10.0 c'est la distance-seuil à laquelle on veut un potentiel égal à 0.1
          pot = -_simweight*exp(-pow(rec,2)/(2*pow(sigma,2)));  
//           cout << "  rec:" <<rec<< " " <<pot << "-";
     }
...
// double SurfaceBasedFunctionalSketchSimilarityModel::prop( const Clique* cl, const std::map<Vertex*, std::string> & changes )
// {
...
void SurfaceBasedFunctionalSketchSimilarityModel::buildTree( Tree & tr ) const
{
  tr.setSyntax( SIA_SURFACEBASEDFUNCTIONALSKETCH_SIMILARITY_MODEL_SYNTAX );
  tr.setProperty( "similarity_weight", _simweight );
  tr.setProperty( "similarity_distance", _simdistance ) ;
//   cout << "TEST : " << _simdistance << endl;

}


void
    SurfaceBasedFunctionalSketchSimilarityModel::buildSimilarity( carto::AttributedObject*
                                              /* parent */,
                                              Tree* ao,
                                              const std::string &
                                              /* filename */ )
{
  float   simweight = 1, simdistance=1.0;
  ao->getProperty( "similarity_weight", simweight );
  ao->getProperty( "similarity_distance", simdistance );
  cout << "simweight :" << simweight << endl;
  cout << "simdistance :" << simdistance << endl;
  SurfaceBasedFunctionalSketchSimilarityModel  *sm
      = new SurfaceBasedFunctionalSketchSimilarityModel( simweight, simdistance );
...
SurfaceBasedFunctionalSketchDataDrivenModel::SurfaceBasedFunctionalSketchDataDrivenModel( float ddweight, float ddh, float ddx1, float ddx2 )
  : Model(), _ddweight( ddweight ), _ddh( ddh ), _ddx1( ddx1 ), _ddx2( ddx2 )
{
}

SurfaceBasedFunctionalSketchDataDrivenModel::~SurfaceBasedFunctionalSketchDataDrivenModel()
{
}

SurfaceBasedFunctionalSketchDataDrivenModel &
    SurfaceBasedFunctionalSketchDataDrivenModel::operator =
    ( const SurfaceBasedFunctionalSketchDataDrivenModel & x )
{
  *(Model *) this = x;
...
Model* SurfaceBasedFunctionalSketchDataDrivenModel::clone() const
{
  return new SurfaceBasedFunctionalSketchDataDrivenModel( *this );
}

double SurfaceBasedFunctionalSketchDataDrivenModel::prop( const Clique* cl )
{
  const VertexClique          *vcl = (const VertexClique *) cl;
  VertexClique::const_iterator     ic, ec = vcl->end();
  set<Vertex *>     vertices;
  ...
  //DEBUG
  //cout << "SurfaceBasedFunctionalSketchDataDrivenModel::prop" << endl;
  ...
  for( ic=vcl->begin(); ic!=ec; ++ic )
  {
      ...
      if( vlabel != "0" )
      {
          (*ic)->getProperty("t", maxInt); // CHOIX DELA  MESURE !!!
...
double SurfaceBasedFunctionalSketchDataDrivenModel::prop( const Clique* cl, const std::map<Vertex*, std::string> & changes )
{
     ...
     for ( ;itChange!=changes.end(); ++itChange )
     {
          ...
          if ((lab != "0") && (oldlab == "0"))
          {
               vert->getProperty("t", maxInt); // CHOIX DELA  MESURE !!!
...
void SurfaceBasedFunctionalSketchDataDrivenModel::buildTree( Tree & tr ) const
{

  tr.setSyntax( SIA_SURFACEBASEDFUNCTIONALSKETCH_DATADRIVEN_MODEL_SYNTAX );
...
void
    SurfaceBasedFunctionalSketchDataDrivenModel::buildDataDriven( carto::AttributedObject*
                                              /* parent */,
                                              Tree* ao,
                                              const std::string &
                                              /* filename */ )
{
  ...
  cout << "ddweight :" << ddweight << endl;
  SurfaceBasedFunctionalSketchDataDrivenModel  *sm
      = new SurfaceBasedFunctionalSketchDataDrivenModel( ddweight, ddh, ddx1, ddx2 );
...
SurfaceBasedFunctionalSketchLowerScaleModel::SurfaceBasedFunctionalSketchLowerScaleModel( float weight )
  : Model(), _lsweight( weight )
{
}


SurfaceBasedFunctionalSketchLowerScaleModel::~SurfaceBasedFunctionalSketchLowerScaleModel()
{
}


SurfaceBasedFunctionalSketchLowerScaleModel &
    SurfaceBasedFunctionalSketchLowerScaleModel::operator =
    ( const SurfaceBasedFunctionalSketchLowerScaleModel & x )
{
  *(Model *) this = x;
...
Model* SurfaceBasedFunctionalSketchLowerScaleModel::clone() const
{
  return new SurfaceBasedFunctionalSketchLowerScaleModel( *this );
}


double SurfaceBasedFunctionalSketchLowerScaleModel::prop( const Clique* cl )
{
  const VertexClique          *vcl = (const VertexClique *) cl;
  VertexClique::const_iterator     ic, ec = vcl->end();
  string            vlabel;
  ...
  //DEBUG
  //cout << "SurfaceBasedFunctionalSketchLowerScaleModel::prop" << endl;
  ...
  //DEBUG
  //cout << "exit" << endl;
//   cout << _lsweight *ns << endl; //*ns*result ;
  return (_lsweight*ns*result);
}

double SurfaceBasedFunctionalSketchLowerScaleModel::prop( const Clique* cl, const std::map<Vertex*, std::string> & changes )
{
     // ici on r�cup�re l'ancien pot, on ne travaille que sur les noeuds qui ont chang�,
     // et on calcule le nouveau pot par diff�rence avec l'ancien
...
void SurfaceBasedFunctionalSketchLowerScaleModel::buildTree( Tree & tr ) const
{

  tr.setSyntax( SIA_SURFACEBASEDFUNCTIONALSKETCH_LOWERSCALEBEST_MODEL_SYNTAX );
  tr.setProperty( "scale_weight", _lsweight );
  cout << "buildTree : " << _lsweight << endl;

}


void
    SurfaceBasedFunctionalSketchLowerScaleModel::buildLowerScale( carto::AttributedObject*
                                              /* parent */,
                                              Tree* ao,
                                              const std::string &
                                              /* filename */ )
{
  ...
  cout << "scale_weight : " << lsweight << endl;

  SurfaceBasedFunctionalSketchLowerScaleModel  *sm
      = new SurfaceBasedFunctionalSketchLowerScaleModel( lsweight );
...
SurfaceBasedFunctionalSketchIntraPSModel::SurfaceBasedFunctionalSketchIntraPSModel( float weight )
  : Model(), _ipsweight( weight )
{
}


SurfaceBasedFunctionalSketchIntraPSModel::~SurfaceBasedFunctionalSketchIntraPSModel()
{
}


SurfaceBasedFunctionalSketchIntraPSModel &
    SurfaceBasedFunctionalSketchIntraPSModel::operator =
    ( const SurfaceBasedFunctionalSketchIntraPSModel & x )
{
  *(Model *) this = x;
...
Model* SurfaceBasedFunctionalSketchIntraPSModel::clone() const
{
  return new SurfaceBasedFunctionalSketchIntraPSModel( *this );
}


double SurfaceBasedFunctionalSketchIntraPSModel::prop( const Clique* cl )
{
...
double SurfaceBasedFunctionalSketchIntraPSModel::prop( const Clique* cl, const std::map<Vertex*, std::string> & changes )
{
     ...
     else
     {
          ...
          for ( ;itC!=(*countTable).end(); ++itC)
          {
               ...
               if (nl < 0)
               {
                 cerr << "Problem in SurfaceBasedfunctionalSketchIntraPSModel::prop() : negative sum of labels" << endl;
                    exit(1);
               }
               else if (nl<=1)
                    pot=0.0;
...
double SurfaceBasedFunctionalSketchIntraPSModel::update( const Clique* cl )
{
     ...
     {
       cout << "SurfaceBasedFunctionalSketchIntraPSModel::update => countTable did not exist..." << endl;
...
// double SurfaceBasedFunctionalSketchIntraPSModel::update( const Clique* cl, const std::map<Vertex*, std::string> & changes )
// {
//      double result=0;
//
//      int ns;
...
/*
  ...
  void SurfaceBasedFunctionalSketchIntraPSModel::buildTree( Tree & tr ) const
{

  tr.setSyntax( SIA_SURFACEBASEDFUNCTIONALSKETCH_INTRAPS_MODEL_SYNTAX );
  tr.setProperty( "intraps_weight", _ipsweight );
}


void
    SurfaceBasedFunctionalSketchIntraPSModel::buildIntraPS( carto::AttributedObject*
                                              /* parent */,
                                              Tree* ao,
                                              const std::string &
                                              /* filename */ )
{
  ...
  cout << "ipsweight :" << ipsweight << endl;
  SurfaceBasedFunctionalSketchIntraPSModel     *sm
      = new SurfaceBasedFunctionalSketchIntraPSModel( ipsweight );
