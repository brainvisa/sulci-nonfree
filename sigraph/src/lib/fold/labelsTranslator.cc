					       ...
					       const string & filename )
{
  ...
  //	marquer ceux � enlever (qui n'ont pas de correspondance dans le MGraph)

  for( il=begin(); il!=fl; ++il )
    if( allowed.find( (*il).second ) == fs )
      {
	//cout << "pas de mod�le pour " << (*il).second << endl;
  ...
  //cout << "\nnb : " << size() << endl;
  //cout << "� enlever : " << toRemove.size() << endl;
...
string FoldLabelsTranslator::lookupLabel( string label ) const
{
  ...
  do
      ...
      if( pos != string::npos )
	{
	  ...
	  //cout << "translate nom compos� --- " << label2 << endl;
	  im = find( label2 );
	  label.erase( 0, pos+1 );
	}
      else
