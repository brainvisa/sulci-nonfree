...
FoldFakeRel::Relmap FoldFakeRel::allocGraph( const CGraph* )
{
  ...
  //	valeurs hors-table � ne pas comptabiliser
...
double FoldFakeRel::update( const CGraph & cg, Relmap & rel )
{
  ...
  //	ensuite les relations trouv�es dans le graphe exemple
  ...
  for( ie=ed.begin(); ie!=fe; ++ie )
	  ...
	  && label2 != SIV_VOID_LABEL && label2 != SIV_BRAIN_HULL )
	{
	  ...
	  // on est s�r que j1 != j2 pcq label1 != label2
	  if( j2 < j1 )
	    {
	      tmp = j2;
	      j2 = j1;
	  ...
	  if( !rd.hasModel )
	    {
	      if( rd.num == 0 )
		++count;	// compte seulement ceux qui passent de 0 � 1
	      ++rd.num;
			  ...
			  const map<Vertex*, string> & changes )
{
  ...
  if( oldpot + res != toto )
    {
      cout << "incoh�rence dans FakeRel: oldpot : " << oldpot << ", chgt : " 
			    ...
			    const map<Vertex*, string> & changes )
{
  ...
  for( iv=changes.begin(); iv!=fv; ++iv )
	...
	if( done.find( edg = *ie ) == df )	// seulement si pas d�j� fait
	  {
	    done.insert( edg );
	    ...
	    ivo = changes.find( v2 );
	    if( ivo != fv )			// autre bout chang� aussi
  ...
  /*if( _ltoi.size() != nl )
    {
      cerr << "FFake rel: nombre de labels chang�. " << nl << " -> " 
	   << _ltoi.size() << endl;
      ASSERT( false );
      }*/
