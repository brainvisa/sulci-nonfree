  ...
  if( tm )
    {
      ...
      if( sl.size() != 0 && vl != "" )	// d�finis
	return( constrainedNoise( cl, outp, sl, vl ) );
    }

  return( openNoise( cl, outp ) );	// tous changements autoris�s
}
					...
					const string & vl )
{
  ...
  // noeuds potentiellement ajout�s, enlev�s, chang�s
  set<Vertex *>			an, rn;
  map<Vertex*, set<string>* >	cn;	// liste de labels prenables associ�e
  ...
  //	tri des noeuds de la clique en 3 cat�gories
  for( iv=((const VertexClique *) cl)->begin(); iv!=fv; ++iv )
      ...
      else
	{
	  ...
	  //	copie ordonn�e (oui: pas optimal)
  ...
  nr = randomGen( rn.size() );	// nb d'enlev�s + chang�s
  if( na + nr == 0 )		// faut que �a fasse au moins 1
    {
      if( an.size() == 0 )
	++nr;
  ...
  nr -= nc;	// ceux qui sont chang�s ne sont pas enlev�s
  ...
  //	enl�vements de labels significatifs (-> void)
  ...
  for( ir=rn.begin(), i=0, it=todo.begin(), ft=todo.end(); it!=ft; ++it )
    {
      //	positionner l'it�rateur de noeuds
      while( i < *it )
	{
	  ++i;
	  ++ir;
  ...
  //	Bon, moi je trouve �a compliqu�, t�tu et tout, comme fonction.
  //	j'ai mal au cr�ne...
