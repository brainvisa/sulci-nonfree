  ...
  try
    {
      ...
      for( i=0; i<ng; ++i )
        {
          ...
          //	erreurs graphe par graphe sur les labels rencontr�s
          ...
          // mettre � jour les arbres inutilis�s (sillons pas pr�sents)
      ...
      for( imt=mtr.begin(), fmt=mtr.end(); imt!=fmt; ++imt )
        {
          ...
          if( scom < 0 )
            {
              cerr << "sigma n�gatif: " << scom << ", taille : " << mcom 
                   << endl;
              scom = 0;
            }
          scom = sqrt( scom );
      ...
      //	si graphe mod�le: mettre aussi les �nergies et les poids
      if( frg )
        {
          float			en, wt, err, wte;
