...
def sort_data_along_subject(I, filename, descr):
    ...
    valid = X[:, 'valid'] == 1
    if valid.sum() < int(X.shape[0] * 0.9):
        return [None] * 4
    if X.shape[1] == 32:
        features = descr[0]
    else:
...
def extract_all_LR(descr, I, prefix):
    ...
    for side in [('Left', 'L')]:
        ...
        for filenameL in globs:
            ...
            if not os.path.exists(filenameR):
                continue
...
def main():
    ...
    descr = [None, None]
    if options.segdescr == 'fd3':
        descr[0] = seg_fd3
    elif options.segdescr == 'fd3_red1':
        descr[0] = seg_fd3_red1
    if options.reldescr == 'fd2':
        descr[1] = rel_fd2
