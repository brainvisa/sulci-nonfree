...
def loadMorpho(wdir):
    ...
    for fn in files:
        if fn.endswith('.dat'):
            ...
            side = l1[2]
            label += '_' + side
            n = numpy.loadtxt(fn, skiprows=1, usecols=list(
                range(3,ncols))).reshape((-1, ncols-3))
            subjects = numpy.loadtxt(fn, skiprows=1, usecols=[
                                     0], dtype=str).reshape((n.shape[0], ))
...
if __name__ == '__main__':
    if sys.argv[1] in ('-h', '--help'):
        print('usage:')
        print(
            sys.argv[0], 'morphomean.dat morphostd.dat subject.arg morphodir property subjectname')
        print()
        sys.exit(0)
