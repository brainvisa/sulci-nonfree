...
def csvPotentials(stats, outpotfile, stds):
    f = open(outpotfile, 'w')
    f.write(
        'subject side label potential normpotential mean_pot stddev_pot numobs_pot\n')
    for sfile, pots in six.iteritems(stats):
        ...
        for slabel, pot in six.iteritems(pots):
            ...
            f.write('%s %s %s %f %f %f %f %d\n' % (subject, side, label,
                                                   pot, npot, cs['mean'], cs['std'], cs['number']))
    f.close()
...
if __name__ == '__main__':
    if sys.argv[1] in ('-h', '--help'):
        ...
        print(
            'if altstats.txt is provided, means/stdev are taken from this alternate source')
        sys.exit(0)
