...
class Tagger(object):
    ...
    def __init__(self, sulcimodel, gaussians_distrib, descriptor,
        ...
        if input_motion:
            self._motion = input_motion * self._motion
        ...
        for i, label in enumerate(self._states):
            ...
            if t_rp:
                translation_prior.append(t_rp[label])
            if d_rp:
                direction_prior.append(d_rp[label])
            if a_rp:
                angle_prior.append(a_rp[label])
    ...
    def _get_priors(self):
        ...
        if labels_prior_distrib:
            ...
            priors = UniformFrequency(self._states_n).frequencies()
...
def parseOpts(argv):
    ...
    parser.add_option('--motion', action='store', dest='motion',
                      ...
                      help='In global mode, output .trm motion file to register ' +
...
def main():
    ...
    if options.vtk:
        ...
        trans, output_available_labels = d.tag(mode, options.eps,
                                               options.maxiter)
    if options.motion:
        if mode == 'global':
            trans.write(options.motion)
        elif mode == 'local':
