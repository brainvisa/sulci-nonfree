...
def compute_depthmap(subject, graph, skel, write_data=False):
    ...
    depth2 = aims.Volume_FLOAT(sX, sY, sZ, sT)
    converter.convert(depth, depth2)
    depth = depth2
    depth[:] /= depth_factor
    if write_data:
        aims.write(depth, '%s_depth.ima' % subject)
    return depth
...
def main():
    ...
    if options.mode == 'subject':
        main_subject(parser, options, args)
    elif options.mode == 'mean':
        main_mean(parser, options, args)
...
def main_subject(parser, options, args):
    ...
    if os.path.exists(depthmapname):
        ...
        depth = compute_depthmap(subject, graph,
