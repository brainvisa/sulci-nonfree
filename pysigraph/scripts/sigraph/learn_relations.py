...
def compute_relations(graphs, distribdir, input_motions,
    ...
    if len(Xd_intra):
        ...
        data['default_intra'] = None, None
    if len(Xd_inter):
        data['default_inter'] = numpy.vstack(Xd_inter), graph_th * 2
    else:
        data['default_inter'] = None, None
    ...
    for relation, (X, count) in data.items():
        if isinstance(relation, tuple):
            ...
            if selected_sulci != None and \
                (sulcus1 not in selected_sulci) \
                    and (sulcus2 not in selected_sulci):
                        continue
        else:
            sulcus1, sulcus2 = None, None
        ...
        if X is None:
            ...
            fd.close()
        if options.savefig:
            save_fig(relation, d, X, filename + '.png')
        d.write(filename)
...
def main():
    ...
    if options.mode == 'normal':
        ...
        if options.sigma_file is None:
            ...
            sigma_file = os.path.join('all', options.sigma_file)
        ...
        for i in range(len(graphs)):
            ...
            if options.sigma_file is None:
                ...
                sigma_file = os.path.join(directory,
                                          options.sigma_file)
