...
class LoopyBeliefPropagation(object):
    ...
    def write_csv(self, csvname):
        ...
        s = 'nodes'
        for si in range(n):
            s += '\tphi_' + self._states[si]
        for si in range(n):
            s += '\tbelief_' + self._states[si]
...
def main():
    ...
    if node_distrib:
        node_model_type = node_distrib['model']
    else:
        node_model_type = 'undefine'
    if rel_distrib:
        rel_model_type = rel_distrib['model']
