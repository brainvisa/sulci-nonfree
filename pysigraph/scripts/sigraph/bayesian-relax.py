...
def clean_available_labels(cgraph, availablelabels, fixed_labels):
	...
	for v in cgraph.vertices():
		...
		for c in todel: c.removeVertex(v)
...
class PythonSulcusOnlyAdaptive(sigraph.AdaptiveLeaf):
	...
	def prop(self, cl):
		...
		if self._node_model: en += w1 * self._node_model.prop(cl)
		vertices = cl.vertices()
		if self._weights['prior']:
			en -= self._weights['prior'] * numpy.log(self._prior)
		return en
...
class PythonDistribSulcusOnlyAdaptive(PythonSulcusOnlyAdaptive):
	...
	def prop_sulcus(self, cl):
		...
		if logli < self._thrsh_li[0]: logli, li = self._thrsh_li
		return - logli
...
def replace_models(datagraph, mgraph, model, prior, weights, nodes_weights):
	...
	for v in mgraph.vertices():
		if v.getSyntax() != 'model_node' : continue
		oldmodel = v['model']
		...
		if nodes_weights: 
			try: node_weight = nodes_weights[sulcus]
			except KeyError: node_weight = 0.
		else:	node_weight = 0.
...
def handle_unknown_model(datagraph, mgraph, nodes_model, prior,
	...
	if nodes_weights:
		try: unknown_weight = nodes_weights['unknown']
		except KeyError: unknown_weight = 0.
	else:	unknown_weight = 0.
	if not nodes_models: return
...
def merge_models(datagraph, mgraph, nodes_model, prior,
	...
	if relations:	merge_relations(mgraph, weights, relations)
...
def main():
	...
	# cleaning
	if options.noweights: remove_weights(an.rGraph())
	if sulci_model or options.norelations: clean_relations(cgraph)
	if availablelabels: clean_available_labels(cgraph,
				availablelabels, fixed_labels)
	
	if sulci_model:
		replace_models(cgraph, an.rGraph(), [nodes_model, sulci_model],
					options.prior, weights, nodes_weights)
	else:	merge_models(cgraph, an.rGraph(), nodes_model, options.prior,
