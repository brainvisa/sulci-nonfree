        ...
        for i, sulcus in enumerate(self._sulci):
            if self._selected_sulci is not None and \
                    sulcus not in self._selected_sulci:
                        continue
            try:
                gd = self._gaussians_distrib['vertices'][sulcus]
            except KeyError:
                continue
            try:
                ld = self._local_distrib['vertices'][sulcus]
            except KeyError:
                continue
            bar.display(i)
        ...
        for i, relation in enumerate(relations):
            ...
            if self._selected_sulci is not None and \
                (label1 not in self._selected_sulci or
                    label2 not in self._selected_sulci):
                        continue
            try:
                gd1 = self._gaussians_distrib['vertices'][label1]
            except KeyError:
                continue
            try:
                gd2 = self._gaussians_distrib['vertices'][label2]
            except KeyError:
                continue
            try:
                ld = self._local_distrib['edges'][relation]
            except KeyError:
                continue
            bar.display(i)
            if label1 != label2:
                ...
                GlobalModelDisplay.display_one(self, label1,
                                               gd1, ld)
...
class GaussianMesher(LocalModelMesher):
    ...
    def mesh(self):
        gaussian = self._ld
        meshes = []
        if gaussian.type() == 'fake_gaussian':
            return
        #mean = gaussian.mean()
...
def mesherFactory(name, options):
    if name in ['kent', 'von_mises_fisher']:
        name = 'orientation'
