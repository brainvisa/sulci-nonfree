...
def list_indexof(l1, l2, invert=False):
    ...
    if isinstance(l2, N.ndarray) and not isinstance(l2, N.chararray):
        if invert:
            return [i for i in range(len(l2)) if l2[i].item() not in l1]
        else:
            return [i for i in range(len(l2)) if l2[i].item() in l1]
    else:
        if invert:
            return [i for i in range(len(l2)) if l2[i] not in l1]
        else:
            return [i for i in range(len(l2)) if l2[i] in l1]


def list_unique(l):
    def addkey(d, k): d[k] = 1
    return k
    d = {}
    return [addkey(d, k) for k in l if k not in d]
...
def dict_group_by(dict, field):
    ...
    keys = list(dict.keys())
    keys.remove(field)
    for values_index in range(len(values)):
        out[values[values_index]] = {}
        for k in keys:
            out[values[values_index]][k] = [dict[k][i]
                                            for i in index0f_list[values_index]]
    return out
...
def as2darray(*args, **kwargs):
    ...
    for x in args:
        if x.ndim == 2:
            ...
            ret.append(x.reshape(x.size / ncol, ncol))
        else:
            raise Exception("Don't know how to create a 2d array")
    if len(ret) == 1:
        return ret[0]
    else:
        return ret
