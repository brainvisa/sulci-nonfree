...
def decodeInNestedList(l, celltypes):
    for i in range(len(l)):
        if isinstance(l[i], list):
            decodeInNestedList(l[i], celltypes)
...
class DataFrame(dmarray):
    ...
    def __new__(subtype, data, dtype="double", copy=True, rownames=None, colnames=None, celltypes=None):
        ...
        if not celltypes is None:
            ...
            new._celltypes = None
        return new
        raise ValuedError("DataFrame could not be constructed",
    ...
    def decode(cls, code):
        ...
        DF.decode([1,3])
        """
        if not N.isscalar(code):
            return [DataFrame.decode(i) for i in code]
        try:
    ...
    def concatenate(self, other, axis=0):
        ...
        try:
            ...
            if axis == out.ndim-1:
                # If concatenation is done along the las axis concatenate the
                # celltypes
    ...
    def __str__(self, quoteStr=False, sep="\t", align=True):
        ...
        for l in [header] + aslist:
            for j in range(len(l)):
                item = l[j]
                if quoteStr and type(item) is str:
                    item = '"'+item+'"'
                else:
                    item = str(item)
    ...
    def write(self, file, quoteStr=True, sep="\t", align=False):
        from datamind import io
        writer = io.WriterCsv()
        writer.write(obj=self, file=file,
                     quoteStr=quoteStr, sep=sep, align=align)
...
if __name__ == "__main__":
    ...
    print(
        'dat[dat[:,"sign"]==DF.code("Bull"), :]------------------------------')
    print("Note that string values should be encoded first")
    print(dat[dat[:, "sign"] == DF.code("Bull"),:])

    print(
        'dat+1 and dat-(dat+1) and dat-dat-----------------------------------')
    print("Note string field are just wrong !!!")
    print(dat + 1)
    print(dat - (dat+1))
    print(dat - dat)
    ...
    print(
        "Quote string & and us a different separator ------------------------")
    print(dat.__str__(sep="#", quoteStr=True))
    ...
    if os.path.exists("small.csv"):
        # Read csv => data.frame
        df = DF.read("small.csv")
        # Select only line of label "X": note that string should be encoded
        # first
        index = df[:, "class"] == DF.code("X")
        d2 = df[index,:]
        print(d2)
    ...
    print(
        "** Concatenation                                                  **")
    print(
        'Concatenation: add a column "height"  ------------------------------')
    d2 = DataFrame(colnames=["height"], data=[[180], [175], [182], [177]])
    d3 = dat.concatenate(d2, axis=1)
    print(d3)
