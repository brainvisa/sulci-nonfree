...
def str2intIndex(index, names=None):
    ...
    if (isinstance(index, list) or isinstance(index, N.ndarray ))\
        and isinstance(index[0], six.string_types):
...
class dmarray(N.ndarray):
    ...
    def __new__(subtype, data, dtype=None, copy=True, rownames=None, colnames=None, dimnames=None):
        ...
        if dimnames is None:
            dimnames = [None] * data.ndim
        if rownames:
            dimnames[0] = N.char.array(rownames)
        if colnames:
            dimnames[1] = N.char.array(colnames)
        if (dtype is None):
            dtype = data.dtype
        ...
        for i in range(len(dimnames)):
            if not dimnames[i] is None:
                new._dimnames[i] = N.char.array(dimnames[i], copy=True)
        return new
    ...
    def formatIndex(self, index):
        ...
        if not self._dimnames is None and isinstance(index, tuple):
            ...
            # index=tuple([str2intIndex(index[i],self._dimnames[i]) for i in
            # xrange(len(index))])
        return index
    ...
    def __getitem__(self, index):
        ...
        if self._dimnames is None or\
            ...
            out._dimnames = []
            for i_dim_self in range(self.ndim):
                if self._dimnames[i_dim_self] is None:
                    ...
                    out._dimnames.append(
                        self._dimnames[i_dim_self][index[i_dim_self]].copy())
    ...
    def rownames(self):
        try:
            return self._dimnames[0]
        except:
            return None

    def colnames(self):
        try:
            return self._dimnames[1]
        except:
            return None
    ...
    def concatenate(self, other, axis=0):
        ...
        try:
            ...
            dimnames[axis] = N.hstack(
                (dimnames[axis], copyIfNotNone(other._dimnames[axis])))
        except:
            dimnames = None
...
if __name__ == "__main__":
    ...
    d = dat[:, N.array([True, True, False, True])]
    print(d, "\n", d.dimnames())
    ...
    print(
        "dat[N.where(dat>=10)]***********************************************")
    d = dat[N.where(dat >= 10)]
    print(d, "\n", d.dimnames())

    prin(
        'dat[:,["tens","thousands"]]*****************************************')
    d = dat[:, ["tens", "thousands"]]
    print(d, "\n", d.dimnames())
    ...
    print(
        'dat[:,N.array(["tens","thousands"])]********************************')
    d = dat[:, N.array(["tens", "thousands"])]
    print(d, "\n", d.dimnames())
