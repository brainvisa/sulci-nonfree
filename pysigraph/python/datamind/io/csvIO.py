...
def slowScan(file, dialect=None, NA=dmc.dmShared.NA_str):
    """
    ...
    header = stripTuple(next(reader))
    ncol = len(header)
    coltypes = [dmc.dmShared.double_str] * ncol
    # print("read -- rows")
    rows = []
    ...
    for row in reader:
        ...
        for j in range(len(row)):
            ...
            # print(j)
            if len(item) == 0 or item == NA:
                item = None
            elif coltypes[j] == dmc.dmShared.double_str:
                try:
                    item = float(item)
                except ValueError:
                    coltypes[j] = dmc.dmShared.string_str
            row[j] = item
        rows.append(row)
...
def fastScan(file, coltypes, dialect=None):
    ...
    # print("read -- rows")
    rows = []
    doubcols = list_indexof([dmc.dmShared.double_str], coltypes)
    for row in reader:
        for col in doubcols:
...
def dict2MinfStr(dict):
    ...
    for i in range(len(keys)):
        string += '    ' + \
            keys[i].__repr__() + ' : ' + dict[keys[i]].__repr__()
        if i < (len(keys) - 1):
            string += ',\n'
        else:
            string += '\n'
        ...
        *args,**kwargs: optional arguments passed to slowScan
        ...
        if fast:
            ...
            if os.path.exists(minffile):
                ...
                attributes = {'data': os.path.basename(
                    file), 'types': None, 'stat': os.lstat(file)}
            if attributes['types']:
                header, rows, coltypes = fastScan(
                    file, coltypes=attributes['types'])
            else:
            ...
            header, rows, coltypes = slowScan(file, *args, **kwargs)
        # print(header, rows, coltypes)
        df = dmc.DataFrame(data=rows, colnames=header, celltypes=coltypes)
...
class WriterCsv(object):
    ...
    def write(self, obj, file, quoteStr=True, sep="\t", align=False):
        f = open(file, "w")
        if isinstance(obj, dmc.DataFrame):
            f.write(obj.__str__(quoteStr, sep, align))
            f.close()
        else:
            raise TypeError("Cannot write data of type '%s'" % type(obj))
