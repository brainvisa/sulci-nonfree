...
def get_sulcus_rotation_axe(vertex):
	...
	o = None
	if omap == 'surface': return hull_normal, vertex['hull_normal_weight']
	if omap == 'buried': refs = ['F.C.L.a.', 'F.C.L.p.']
	elif omap == 'broca': refs = ['INSULA']
	dirs = []
	sizes = []
	for e in vertex.edges():
		if e.getSyntax() != 'junction': continue
		v1, v2 = e.vertices()
		if v2['name'] == vertex['name'] : v2, v1 = v1, v2
		rootlabel2 = re.sub("_(left|right)", '', v2['name'])
		if rootlabel2 not in refs: continue
		try: dir = e['refdirection'].arraydata()
		except KeyError: continue
		dirs.append(dir)
		sizes.append(e['refsize'])
	if not len(sizes): return None
	w = numpy.sum(sizes)
...
def reorient(dirs, mean_dir=None):
	if mean_dir is None: return dirs
