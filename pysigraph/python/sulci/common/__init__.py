...
def unlock(filename):
	...
	except OSError: pass # allready unlock
...
def lock(filename, timeout=60):
	...
	fd.close()
	while 1:
		try:	os.link(tmp, filename) # atomic on NFS
		except OSError: pass # ignore errors (known bug on NFS)
		tmp_stats = os.stat(tmp)
		if tmp_stats[3] == 2:
			os.unlink(tmp)
			return
		try:	lock_stats = os.stat(filename)
		except OSError:
			time.sleep(0.1)
			continue
