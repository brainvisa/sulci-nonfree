...
class UniformFrequency(Frequency):
    def __init__(self, dim=None):
        ...
        if dim:
            self._freq = numpy.asmatrix(numpy.ones(dim))
...
by dichotomy
    ...
    def _compute_scale(self, X, weights):
        if weights is None:
            ...
            return (numpy.dot(weights, X) / self._shape)
...
P(x|l) = l * e^(- l * x)
    ...
    def fit(self, X, weights=None):
        ...
        if weights is None:
            ...
            self._lambda = 1. / numpy.dot(weights, X)
...
p :      dimensionality of data
    ...
    def _check(self, X):
        if (((X ** 2).sum(axis=1) - 1) ** 2 > 10e-5).any():
            return 1
...
mu = E(X) / || E(X) ||
        ...
        if weights is None:
            ...
            R = numpy.dot(weights * X.shape[0] / weights.sum(), X)
...
f(x) = -----------------------------------------------------
    ...
    def update(self):
        ...
        if b:
            ...
            self._normalization = k / (4 * numpy.pi * numpy.sinh(k))
        if b:
            self._lognormalization = -k + 0.5 * \
                    (numpy.log(k - 2 * b) + numpy.log(k + 2 * b)) -\
                     numpy.log(2 * numpy.pi)
        else:
            self._lognormalization = numpy.log(k) - \
...
ref : "Equatorial Distributions on a Sphere", G.S. Watson
  ...
  interpolation.
        ...
        if not None in [mu, kappa]:
            ...
            self._normalization = self._lognormalization = None
...
class GammaExponentialMixtureModel(MixtureModel):
    ...
    def em(self, X, maxiter=1000, eps=10e-5):
        ...
        while 1:
            ...
            n += 1
            if (n > 2 and old_en - en < eps) or n >= maxiter:
                break
            old_en = en
...
Return Value : BIC
        ...
        if W is not None:
            ...
            li = numpy.sum(numpy.log(L.sum(axis=0)))
