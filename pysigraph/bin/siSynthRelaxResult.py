...
def old_read_csv_result(filename):
	def check_data(data_list):
		...
		if len(error_msg) > 0:
			...
			for e in error_msg: sys.stderr.write('\t' + e)
			return {}
		else:	return kept_lines
...
def old_compute_all_info(filename, details):
	...
	for (n, db) in [('train', train), ('test', test), ('gen', gen)]:
		...
		for (subject, correlation) in subject_cor.items():
			if numpy.isnan(correlation): continue
			msg.write('\t%s\t%f (%d)\n' % (subject,
		...
		for (subject, (rate, energy)) in subject_std.items():
			if numpy.isnan(rate) and numpy.isnan(energy): continue
			msg.write('\t%s\t%f (%d)\t%f (%d)\n' % \
	...
	for name in db:
		...
		if len(results) == 0: skipped += [name]
			...
			if len(local_energies) == 0:
				energies += [numpy.nan]
			else:	energies += [numpy.array(local_energies).mean()]
	a = numpy.array(rates)
...
def compute_mean(db, mode, show_std):
	...
	if mode == 'mean': db_subject = mean_each_subject(db)
	elif mode == 'best': db_subject = min_each_subject(db)
	...
	else:	means = [all_mean, train_mean, test_mean, gen_mean]
	...
	if show_std:
		types = ['all_mean', 'all_std', 'train_mean', 'train_std',
			'test_mean', 'test_std', 'gen_mean', 'gen_std']
	else:	types = ['all_mean', 'train_mean', 'test_mean', 'gen_mean']
...
def process(f, options):
	if options.old:
		if options.show_mean: old_compute_all_info(f, 'short')
		...
		if options.show_correlation: r += [compute_correlations(db)]
		res = numpy.vstack(r)
		res = numpy.hstack((numpy.zeros((res.shape[0], 2)),res))
		res = datamind.core.DataFrame(data=res,
			colnames=['File', 'Base'] + r[0].colnames().tolist(),
