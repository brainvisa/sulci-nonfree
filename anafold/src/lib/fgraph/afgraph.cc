...
void AFGraph::initCliques()
{
  ...
  mg->removeWeights(); // added 2012/02/21 to match standard annealing
                       // conditions
  mg->modelFinder().initCliques( *cg, true, false, true );
  _folds->setChanged();
  ...
  const CGraph::CliqueSet	&cliques = cg->cliques();
  CGraph::CliqueSet::const_iterator	ic, fc=cliques.end();
  string	label, label2;
	  ...
	  if( al )
	    {
	      ...
	      if( _normInpVec )	// normer les entr�es ?
		{
